 ...
 * genvar.m Matlab wrapper function (utils subfolder); all error checking and
	...
	if (cvec) { /* apply c to each row in turn */
		if (n == 1) { /* y is c vector: special case */
			...
			for (size_t i=0; i<n; ++i) {
				...
				for (size_t t=0; t<mmp; ++t) {
					double yvar = 0.0;
					for (size_t k=0; k<t; ++k) {
						yvar += c[k]*yi[n*(t-(k+1))]; /* lag is k+1 !!! */
					}
					yi[n*t] += yvar;
				}
				for (size_t t=mmp; t<m; ++t) {
		...
		for (size_t t=0; t<mmp; ++t) {
			...
			for (size_t k=0; k<t; ++k) {
				...
				const double* const ylag = yt-n*(k+1); /* lag is k+1 !!! */
				for (size_t i=0; i<n; ++i) {
