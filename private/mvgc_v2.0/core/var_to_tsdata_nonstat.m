...
%     V          time-varying residuals covariance matrix
...
%% Description
%
% Return |N| independent random Gaussian VAR time series, with time-varying
% coefficients |A| and residuals covariances |V|. The last index in |A| and
% |V| is the time index (the number of observations is taken as the size of
% the last dimension). If |V| is 2 dimensional, then it is replicated at each
% time.
%
...
function X = var_to_tsdata_nonstat(A,V,N)
...
for r = 1:N % for each realization
    X(:,:,r) = genvar_nonstat(A,V,n,p,m,statsig);
end

function X = genvar_nonstat(A,V,n,p,m,statsig)

% initialise to Gaussian white noise

if statsig
    X = V*randn(n,m); % "V" is actually Cholesky matrix
else
    X = zeros(n,m);
    for t = 1:m % for each time step
        [C,cholp] = chol(V(:,:,t),'lower');
