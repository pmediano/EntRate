...
%
% <matlab:open('tsdata_to_cpsd_v1.m') code>
%
%% Syntax
%
%     S = tsdata_to_cpsd_v1(X,fres,method,window,noverlap,nw,ntapers)
%
%% Arguments
%
...
%     fres       frequency resolution (default: number of observations)
%     method     estimation method: 'WELCH' (default) or 'MT'
%     window     window length (default: 1/8 number of observations)
...
function [S,fres] = tsdata_to_cpsd_v1(X,fres,method,window,noverlap,nw,ntapers)
...
if nargin < 2 || isempty(fres)
    fres = m-1; % added v2.0 - frequency resolution defaults to number of observations
end
...
%   window = min(m,nfft); % according to Chronux ... by default Matlab 'pwelch' splits data into 8 overlapping segments
    window = round(m/8); % v2.0 - go with Matlab default!
end
...
if strcmpi(method,'MT')
    ...
    S = pi*permute(S,[2 3 1])/N; % FIX ME - SCALING IS NOT CORRECT!!!
