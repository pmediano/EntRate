...
function FE = empirical_var_to_pwcgc(A,SIG,m,N,H0,nsamps,mtrunc,decayfac,regmode,~,~) % v2.0 - last two parameters no longer required
...
if H0 % null hypothesis: no connectivity for each j -> i
        ...
        for i = 1:n
            if i ~= j
                ...
                for s = 1:nsamps
                    ...
                    FE(s,i,j) = ss_to_mvgc(AE1,CE,KE,SIGE,i,j);
    ...
    for s = 1:nsamps
        ...
        [AE1,CE,KE,res] = var_to_ss(AE,SIGE,2);
        if res.error, fprintf(2,' *** %s\n',res.errmsg); continue; end
        FE(s,:,:) = ss_to_pwcgc(AE1,CE,KE,SIGE);
