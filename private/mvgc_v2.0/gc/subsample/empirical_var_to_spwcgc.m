...
function fE = empirical_var_to_spwcgc(A,SIG,m,N,fres,H0,nsamps,mtrunc,decayfac,regmode,~,~) % v2.0 - last two parameters no longer required
...
if H0 % null hypothesis: no connectivity for each j -> i
        ...
        for i = 1:n
            if i ~= j
                ...
                for s = 1:nsamps
                    ...
                    % calculate empirical spectral PWGC

                    [AE1,CE,KE,res] = var_to_ss(AE,SIGE,2);
                    if res.error, fprintf(2,' *** %s\n',res.errmsg); continue; end
                    fE(s,i,j,:) = ss_to_smvgc(AE1,CE,KE,SIGE,i,j,fres);
    ...
    for s = 1:nsamps
        ...
        [AE1,CE,KE,res] = var_to_ss(AE,SIGE,2);
        if res.error, fprintf(2,' *** bad SS\n'); continue; end
        if res.error, fprintf(2,' *** %s\n',res.errmsg); continue; end
        fE(s,:,:,:) = ss_to_spwcgc(AE1,CE,KE,SIGE,fres);
