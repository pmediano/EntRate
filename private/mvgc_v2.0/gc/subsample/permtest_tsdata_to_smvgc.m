...
function fP = permtest_tsdata_to_smvgc(U,x,y,p,fres,bsize,nsamps,regmode,~,~) % v2.0 - last two parameters no longer required
...
for s = 1:nsamps
    ...
    % calculate permutation test spectral MVGC
    
    [AP1,CP,KP,res] = var_to_ss(AP,SIGP,2);
    if res.error, fprintf(2,' *** %s\n',res.errmsg); continue; end
    fP(s,:) = ss_to_smvgc(AP1,CP,KP,SIGP,x,y,fres);
