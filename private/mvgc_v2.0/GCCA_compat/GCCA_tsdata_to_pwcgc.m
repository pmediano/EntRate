...
if ~isempty(wmsg), fprintf(2,'WARNING in full regression (data non-stationary or colinear?): %s\n',wmsg); end
if isbad(SIG),     fprintf(2,'ERROR in full regression (check output of ''var_info'')\n'); return; end % show-stopper!
% we should probably check that SIG is positive-definite
...
for j = 1:n;
    ...
    wmsg = lastwarn; warning(wstate);
    if ~isempty(wmsg), fprintf(2,'WARNING in reduced regression for source node %d (data non-stationary or colinear?): %s\n',j,wmsg); end
    if isbad(SIGj),    fprintf(2,'ERROR in reduced regression for source node %d (data non-stationary or colinear?)\n',j); continue; end % show-stopper!
