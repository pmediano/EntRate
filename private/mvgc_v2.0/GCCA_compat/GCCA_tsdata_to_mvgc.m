...
% we should probably check that V is positive-definite

% reduced regression

wstate = warning('off','all'); lastwarn('');
[~,VR] = tsdata_to_var(U(r,:,:),pp,regmode);
wmsg = lastwarn; warning(wstate);
if ~isempty(wmsg), fprintf(2,'WARNING in full regression (data non-stationary or colinear?): %s\n',wmsg); end
if isbad(VR),      fprintf(2,'ERROR in full regression (check output of ''var_info'')\n'); return; end % show-stopper!
% we should probably check that V is positive-definite
