...
%     auto       only plot auto- (not cross-) correlation (default: true)
...
% autocovariance is plotted (see <autocov2autocorr.html |autocov2autocorr|>). If |acorr| is
...
% <autocov2autocorr.html |autocov2autocorr|>
%
...
if ~isempty(trange)
    assert(isvector(trange) && length(trange) == 2 && trange(1) < trange(2),'time range must be an ascending 2-vector of times');
end
...
if acorr, for s = 1:N, G(:,:,:,s) = autocov2autocorr(G(:,:,:,s)); end; end
