...
%% Syntax
%
%     [xup,xlo] = mvgc_confint(alpha,x,p,m,N,nx,ny,nz)
%
%% Arguments
%
...
%% Description
%
% Return confidence interval |[xup,xlo]| at significance level |alpha| for
% sample MVGCs in |x|, based on theoretical (asymptotic) distributions. |NaN| s
% are ignored. See <mvgc_cdfi.html |mvgc_cdfi|> for details of other parameters.
%
%% References
%
...
function [xup,xlo] = mvgc_confint(alpha,x,p,m,N,nx,ny,nz,~)
...
a = alpha*ones(size(x));
xup(nn) = mvgc_cdfi(1-a,x,p,m,N,nx,ny,nz);
xlo(nn) = mvgc_cdfi(a,  x,p,m,N,nx,ny,nz);
